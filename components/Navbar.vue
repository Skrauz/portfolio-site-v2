...
<template>
  <nav
    class="common-container w-full flex md:items-end items-center justify-between md:py-12 py-6"
  >
    <!-- Logo -->
    ...
    <button
      ...
      class="md:hidden p-2 cursor-pointer transition-colors hover:text-theme-light"
      aria-label="Toggle Menu"
    >
      <svg
        xmlns="http://www.w3.org/2000/svg"
        class="h-8 w-8 text-text-dark-1"
        fill="none"
        viewBox="0 0 24 24"
        stroke="currentColor"
      >
  ...
  >
    <div
      ...
      @click="toggleMobileMenu"
      class="mobile-menu-backdrop fixed top-0 left-0 w-full h-screen bg-black/50 z-[45]"
    ></div>
  </Transition>
  <Transition
    name="fade"
    enter-active-class="duration-150 ease-out"
    enter-from-class="opacity-0 -translate-x-36"
    enter-to-class="opacity-100 translate-x-0"
    leave-active-class="transition-all duration-150 ease-in"
    leave-from-class="opacity-100 translate-x-0"
    leave-to-class="opacity-0 -translate-x-36"
  >
    <nav
      v-if="isMobileMenuOpen"
      class="mobile-menu fixed top-0 left-0 w-[75%] h-screen bg-theme-dark-2 z-50"
    >
      <div class="ml-2">
        <div
          class="common-container flex items-center justify-between md:py-12 py-6"
        >
          ...
          <button
            @click="toggleMobileMenu"
            class="p-2 cursor-pointer transition-colors hover:text-theme-light"
        ...
        <div class="py-6 common-container absolute bottom-0 left-0">
          <div class="flex items-center gap-4">
            ...
            <nuxt-link
              ...
              to="mailto:krauzowiczs42@gmail.com"
              target="_blank"
            >
