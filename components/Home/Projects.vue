      ...
      <div class="w-full">
        ...
        <div class="projects gap-6 mt-6 md:mt-12 xl:mt-20 grid grid-cols-1 lg:grid-cols-2">
          <div class="project md:p-8 p-7 relative group h-max" v-for="(project, idx) in projects" :key="project.name">
            ...
            <nuxt-img src="/img/frame_small.svg" class="-z-10 absolute top-0 left-0"></nuxt-img>
            <template v-if="project.link">
              <nuxt-link :to="project.link" target="_blank"
                class="project-header hover:text-theme-accent flex items-center gap-3 max-w-max transition-colors">
                <h3 class="project-name text-2xl md:text-3xl font-bold leading-[0.9] tracking-[-2px] text-center"> {{
                  project.name }} </h3>
                <div class="w-6">
            ...
            <template v-else>
              <div class="project-header hover:text-theme-accent flex items-center gap-3 max-w-max transition-colors">
                <h3 class="project-name text-2xl md:text-3xl font-bold leading-[0.9] tracking-[-2px] text-center"> {{
                  project.name }} </h3>
              </div>
            </template>
            <div class="project-body flex xl:flex-row flex-col gap-4 mt-4">
              <p class="project-description leading-snug">{{ project.description }}</p>
              <div v-if="project.image" class="w-full xl:w-[40%] max-w-xs shrink-0">
                <nuxt-img class="w-full rounded-sm" :src="project.image"></nuxt-img>
              </div>
            </div>
            <div class="project-information mt-6">
              <div class="grid grid-cols-2 gap-4">
                <div class="col-span-2">
                  <h4 class="text-2xl font-bold leading-[0.9] tracking-[-2px]">Technologie</h4>
